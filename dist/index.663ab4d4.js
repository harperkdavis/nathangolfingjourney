...
async function connect() {
    ...
    game.socket.on("world", (data)=>{
        ...
        temp.played = {};
        if (game.world.balls[id] && game.world.balls[id].ball) game.selfBall = game.world.balls[id].ball;
...
function keyPressed() {
    if (!game.started) {
        if (getAdjective()) {
            ...
            if (keyCode == BACKSPACE) {
                if (temp.usernameTyped.length > 0) temp.usernameTyped = temp.usernameTyped.substring(0, temp.usernameTyped.length - 1);
            }
            if (keyCode == ENTER) {
    ...
    if (keyCode == BACKSPACE) {
        if (game.chatting) {
            if (temp.typing.length > 0) temp.typing = temp.typing.substring(0, temp.typing.length - 1);
        }
    }
}
...
function drawLevel(level, offset) {
    ...
    for (let v of level.points){
        ...
        if (v.y > 99) gaps.push(v.x + offset);
    }
    ...
    if (game.selfBall) {
        if (game.selfBall.level == level.id) {
            if (level.stats) {
                ...
                if (level.holeY > 50) posY = level.holeY + 5;
                noStroke();
                fill(0);
                textSize(3 * SCALE);
                textAlign(LEFT, TOP);
...
function drawAim(input, color = [
    ...
    line((input.aimX - input.aimVelX * 1.5) * SCALE, (input.aimY - input.aimVelY * 1.5) * SCALE, (input.aimX - input.aimVelX * max(40 * input.aimPower, 1.5)) * SCALE, (input.aimY - input.aimVelY * max(40 * input.aimPower, 1.5)) * SCALE);
    stroke(color[0], color[1], color[2], min(input.aimPower * 2000, 100 * transparency));
    line((input.aimX + input.aimVelX * 1.5) * SCALE, (input.aimY + input.aimVelY * 1.5) * SCALE, (input.aimX + input.aimVelX * max(40 * input.aimPower, 1.5)) * SCALE, (input.aimY + input.aimVelY * max(40 * input.aimPower, 1.5)) * SCALE);
    line((input.aimX + input.aimVelY + input.aimVelX * max(40 * input.aimPower, 1.5)) * SCALE, (input.aimY - input.aimVelX + input.aimVelY * max(40 * input.aimPower, 1.5)) * SCALE, (input.aimX - input.aimVelY + input.aimVelX * max(40 * input.aimPower, 1.5)) * SCALE, (input.aimY + input.aimVelX + input.aimVelY * max(40 * input.aimPower, 1.5)) * SCALE);
    noStroke();
    fill(color[0], color[1], color[2], min(input.aimPower * 3000, 255 * transparency));
    triangle((input.aimX - input.aimVelX * max(40 * input.aimPower, 1.5)) * SCALE, (input.aimY - input.aimVelY * max(40 * input.aimPower, 1.5)) * SCALE, (input.aimX + input.aimVelY * 0.5 + input.aimVelX * 1.5 - input.aimVelX * max(40 * input.aimPower, 1.5)) * SCALE, (input.aimY - input.aimVelX * 0.5 + input.aimVelY * 1.5 - input.aimVelY * max(40 * input.aimPower, 1.5)) * SCALE, (input.aimX - input.aimVelY * 0.5 + input.aimVelX * 1.5 - input.aimVelX * max(40 * input.aimPower, 1.5)) * SCALE, (input.aimY + input.aimVelX * 0.5 + input.aimVelY * 1.5 - input.aimVelY * max(40 * input.aimPower, 1.5)) * SCALE);
}
function draw() {
    if (!game.started) {
        background(0);
        ...
        if (!isLoggedIn()) {
            ...
            if (!getAdjective()) {
                ...
                if (temp.adjectiveChoice == undefined) temp.adjectiveChoice = floor(random(0, constant.adjectives.length));
                ...
                if (mouseIsPressed) {
                    if (isWithin(mouseX, mouseY, width / 2 + 10, height / 2 - 15, 10, 10)) {
                        ...
                        if (temp.adjectiveChoice >= constant.adjectives.length) temp.adjectiveChoice = 0;
                    } else if (isWithin(mouseX, mouseY, width / 2 - 20, height / 2 - 15, 10, 10)) localStorage.setItem("nathan_golfing_journey_adjective", temp.adjectiveChoice);
                }
                return;
            }
            ...
            text("This is the " + getAdjective() + " journey of", width / 2, height / 2 - 80);
            textSize(64);
            if (game.selfBall) fill(game.selfBall.color[0], game.selfBall.color[1], game.selfBall.color[2]);
            else fill(255);
            ...
            if (mouseIsPressed && isWithin(mouseX, mouseY, width / 2 - 50, height / 2 + 10, 100, 20)) game.started = true;
        }
        return;
    }
    update();
    ...
    if (game.world != undefined && game.prevWorld != undefined) {
        ...
        if (game.selfBall) {
            ...
            if (player && prevPlayer) {
                ...
                if (game.selfBall.trans >= 0) translate(-game.levels[1].width * ((40 - x) / 40) * SCALE, 0);
            }
        }
        if (game.levels) {
    ...
    if (game.input.aiming) {
        if (game.selfBall) drawAim(game.input, [
            0,
            0,
            0
        ], game.selfBall.canStroke && game.selfBall.trans < 0 ? 1 : 0.4);
    }
    pop();
    if (game.selfBall) {
